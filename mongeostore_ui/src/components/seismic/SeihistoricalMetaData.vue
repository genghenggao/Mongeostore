 ...
 * @LastEditors: henggao
 * @LastEditTime: 2022-05-13 14:50:58
-->
<template>
  <div class="DataShow">
    <div>
      <h2
        ...
        "
      >
        <i class="el-icon-caret-right" /> 地震历史数据
      </h2>
    </div>
    <el-container>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  size="mini"
                  @click="
                    downloadfile(scope.row.seihistorical_filename, scope.row.id)
                  "
                  >下载文件</el-button
                >
              </template>
...
export default {
  ...
  methods: {
    ...
    showData(n1, n2) {
      const url = "http://127.0.0.1:8000/seismic/seihistoricalinfo/";
      axios
        .get(url, {
          params: {
            // 每页显示的条数
    ...
    // 上传
    uploadSeihistoricalData() {
      this.$router.push({ path: "/mongeostore/seihistoricalupload" });
    },
    // 下载
    downloadfile(seihistorical_filename, id) {
      console.log(id);
      const url = "http://127.0.0.1:8000/seismic/seihistoricalfiledownload/";
      axios
        .get(url, {
          params: {
            file_id: id,
        ...
        .then((response) => {
          if (response.data.size === 0) {
            ...
            link.href = url;
            link.download = seihistorical_filename; // 重命名文件
            link.click();
    ...
    handleEdit(index, row) {
      ...
      if (row.isEdit) {
        ...
        const url = "http://127.0.0.1:8000/seismic/editseihistoricalinfo/";
        axios
          .post(
            url,
            {
    ...
    deleteRow(index, rows, row) {
      ...
      this.$confirm("永久删除，是否继续？", "提示", {
        ...
        .then(() => {
          ...
          const url = "http://127.0.0.1:8000/seismic/deleteseihistoricalinfo/";
          axios
            .post(
              url,
              {
    ...
    onSearchSubmit(n1, n2) {
      ...
      if (this.searchCondition.filter_key == "") {
        ...
        const url = "http://127.0.0.1:8000/seismic/seihistoricalinfosearch/";
        axios
          .get(url, {
            params: {
              // 每页显示的条数

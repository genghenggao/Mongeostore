  ...
  <div class="DataShow">
    <div>
      <h2
        ...
        "
      >
        <i class="el-icon-caret-right" /> 地震处理数据
      </h2>
    </div>
    <el-container>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  size="mini"
                  @click="
                    downloadfile(scope.row.seiprocess_filename, scope.row.id)
                  "
                  >下载文件</el-button
                >
              </template>
...
export default {
  ...
  methods: {
    ...
    showData(n1, n2) {
      const url = "http://127.0.0.1:8000/seismic/seiprocessinfo/";
      axios
        .get(url, {
          params: {
            // 每页显示的条数
    ...
    // 上传
    uploadSeiprocessData() {
      this.$router.push({ path: "/mongeostore/seiprocessupload" });
    },
    // 下载
    downloadfile(seiprocess_filename, id) {
      console.log(id);
      const url = "http://127.0.0.1:8000/seismic/seiprocessfiledownload/";
      axios
        .get(url, {
          params: {
            file_id: id,
    ...
    deleteRow(index, rows, row) {
      ...
      this.$confirm("永久删除，是否继续？", "提示", {
        ...
        .then(() => {
          ...
          const url = "http://127.0.0.1:8000/seismic/deleteseiprocessinfo/";
          axios
            .post(
              url,
              {

 ...
 * @LastEditors: henggao
 * @LastEditTime: 2022-05-13 15:05:11
-->
<template>
  <div class="DataShow">
    <div>
      <h2
        ...
        "
      >
        <i class="el-icon-caret-right" /> 地震解释数据
      </h2>
    </div>
    <el-container>
          ...
          >
            ...
            <el-form-item id="addNew-item">
              <el-button
                ...
                plain
                icon="el-icon-upload"
                @click="uploadSeiInterpretationData"
                >上传</el-button
              >
            </el-form-item>
          </el-form>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  @click="
                    downloadfile(
                      scope.row.seiInterpretation_filename,
                      scope.row.id
                    )
                  "
                  >下载文件</el-button
...
export default {
  ...
  methods: {
    ...
    showData(n1, n2) {
      const url = "http://127.0.0.1:8000/seismic/seiInterpretationinfo/";
      axios
        .get(url, {
          params: {
            // 每页显示的条数
    ...
    // 上传
    uploadSeiInterpretationData() {
      this.$router.push({ path: "/mongeostore/seiInterpretationupload" });
    },
    // 下载
    downloadfile(seiInterpretation_filename, id) {
      console.log(id);
      const url =
        "http://127.0.0.1:8000/seismic/seiInterpretationfiledownload/";
      axios
        .get(url, {
          params: {
            file_id: id,
        ...
        .then((response) => {
          if (response.data.size === 0) {
            ...
            link.href = url;
            link.download = seiInterpretation_filename; // 重命名文件
            link.click();
    ...
    handleEdit(index, row) {
      ...
      if (row.isEdit) {
        ...
        const url = "http://127.0.0.1:8000/seismic/editseiInterpretationinfo/";
        axios
          .post(
            url,
            {
    ...
    deleteRow(index, rows, row) {
      ...
      this.$confirm("永久删除，是否继续？", "提示", {
        ...
        .then(() => {
          ...
          const url =
            "http://127.0.0.1:8000/seismic/deleteseiInterpretationinfo/";
          axios
            .post(
              url,
              {
    ...
    onSearchSubmit(n1, n2) {
      ...
      if (this.searchCondition.filter_key == "") {
        ...
        const url =
          "http://127.0.0.1:8000/seismic/seiInterpretationinfosearch/";
        axios
          .get(url, {
            params: {
              // 每页显示的条数

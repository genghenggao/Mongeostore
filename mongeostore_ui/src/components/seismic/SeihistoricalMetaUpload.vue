 ...
 * @LastEditors: henggao
 * @LastEditTime: 2022-05-13 15:02:09
-->
<template>
  <el-row :gutter="20">
    <el-col :span="10" :offset="6">
      <el-container style="min-width: 600px; overflow: hidden">
        <el-header><h2>地震历史数据上传</h2></el-header>
        <el-main>
          <el-form
            ref="form"
            :model="form"
          ...
          >
            <el-scrollbar style="height: 650px">
              <el-form-item label="文件名称">
                <el-input
                  v-model="form.seihistorical_filename"
              ...
              <el-form-item label="上传时间">
                <el-date-picker
                  type="date"
                  placeholder="选择日期"
                  v-model="form.seihistorical_upload_date"
                  style="width: 100%"
                ></el-date-picker
              ></el-form-item>
...
export default {
  name: "SeihistoricalMetaUpload",
  data() {
    return {
      form: {
        seihistorical_filename: "",
        location: "",
        company_name: "",
        uploader: "",
        depth: "",
        project_name: "",
        seihistorical_upload_date: "",
      },
      show: false,
      fileList: [],
      fileOptions: {
        browse_button: "VideoChose",
        url: "http://127.0.0.1:8000/seismic/seihistoricalinfo/",
  ...
  mounted() {
    ...
    //获取uuid
    let url = `http://127.0.0.1:8000/seismic/seihistoricalinfo/`;
  ...
  methods: {
    ...
    BeforeUpload(uploader, file) {
      this.fileList = this.fileList.map((val, ind) => {
        ...
        if (
          this.form["seihistorical_filename"] == "" ||
          this.form["location"] == "" ||
          this.form["project_name"] == "" ||
          this.form["company_name"] == "" ||
          this.form["uploader"] == "" ||
          this.form["seihistorical_upload_date"] == ""
        ) {
          console.log("信息有误！");
        } else {
          uploader.setOption("multipart_params", {
            seihistorical_filename: this.form["seihistorical_filename"],
            location: this.form["location"],
            project_name: this.form["project_name"],
            company_name: this.form["company_name"],
            uploader: this.form["uploader"],
            seihistorical_upload_date: this.form["seihistorical_upload_date"].getTime(),
            filename: val.name,

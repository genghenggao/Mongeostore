  ...
  <div class="DataShow">
    <div>
      <h2
        ...
        "
      >
        <i class="el-icon-caret-right" /> 测井数据信息
      </h2>
    </div>
    <el-container>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  size="mini"
                  @click="
                    downloadfile(scope.row.logging_filename, scope.row.id)
                  "
                  >下载文件</el-button
                >
              </template>
...
export default {
  ...
  methods: {
    ...
    showData(n1, n2) {
      const url = "http://127.0.0.1:8000/seismic/logginginfo/";
      axios
        .get(url, {
          params: {
            // 每页显示的条数
    ...
    // 上传
    uploadLoggingData() {
      this.$router.push({ path: "/mongeostore/loggingupload" });
    },
    // 下载
    downloadfile(logging_filename, id) {
      console.log(id);
      const url = "http://127.0.0.1:8000/seismic/loggingfiledownload/";
      axios
        .get(url, {
          params: {
            file_id: id,
        ...
        .then((response) => {
          if (response.data.size === 0) {
            ...
            // link.download = logging_filename + ".segy"; // 重命名文件
            link.download = logging_filename; // 重命名文件
            link.click();
    ...
    handleEdit(index, row) {
      ...
      if (row.isEdit) {
        ...
        const url = "http://127.0.0.1:8000/seismic/editlogginginfo/";
        axios
          .post(
            url,
            {
    ...
    deleteRow(index, rows, row) {
      ...
      this.$confirm("永久删除，是否继续？", "提示", {
        ...
        .then(() => {
          ...
          const url = "http://127.0.0.1:8000/seismic/deletelogginginfo/";
          axios
            .post(
              url,
              {
    ...
    onSearchSubmit(n1, n2) {
      ...
      if (this.searchCondition.filter_key == "") {
        ...
        const url = "http://127.0.0.1:8000/seismic/logginginfosearch/";
        axios
          .get(url, {
            params: {
              // 每页显示的条数
